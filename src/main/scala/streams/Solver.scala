...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    b.legalNeighbors.to(LazyList).map { case (block, move) => (block, move :: history) }
  }
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter { case (b, _) => done(b) }
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.headOption.map(_._2.reverse).getOrElse(List.empty)